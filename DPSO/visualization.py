...
from common.config import GRID_RESOLUTION, MAP_HEIGHT, MAP_WIDTH


GENERATED_GRAPHS_DIR = Path(__file__).resolve().parent / "generated_graphs"
...
def _graph_dir(output_dir=None):
    path = Path(output_dir) if output_dir else GENERATED_GRAPHS_DIR
...
def plot_assignment_map(ax, uavs, tasks, result, demand_map, label="DPSO"):
    ax.imshow(
        demand_map,
        origin="lower",
        extent=[0, MAP_WIDTH * GRID_RESOLUTION, 0, MAP_HEIGHT * GRID_RESOLUTION],
...
def plot_trajectories(ax, uavs, result, demand_map, label="DPSO"):
    ax.imshow(
        demand_map,
        origin="lower",
        extent=[0, MAP_WIDTH * GRID_RESOLUTION, 0, MAP_HEIGHT * GRID_RESOLUTION],
...
def plot_dpso_convergence(ax, result, label="DPSO"):
    if not result.history:
        ax.text(0.5, 0.5, f"No {label} history available", ha="center", va="center", transform=ax.transAxes)
    else:
        ax.plot(result.history, color="#9467bd", linewidth=1.8)
    ax.set_xlabel("Iteration")
    ax.set_ylabel("Best fitness")
    ax.set_title(f"{label} Pheromone-DPSO Fitness Convergence")
    ax.grid(linestyle="--", alpha=0.45)


def save_all_graphs(uavs, tasks, demand_map, result, output_dir=None, prefix="dpso_", label=None):
    ...
    plot_dpso_convergence(axes[2], result, label=label)
