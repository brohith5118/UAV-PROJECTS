...
from scheduler import DPSOScheduler
...
def print_summary(result, tasks, uavs, label="DPSO"):
    scheduled_events = _scheduled_events(result)
    ...
    print("-" * 72)
    for task_type, label_text in [(-1, "Acquisition-only"), (0, "Light compute"), (1, "Compute-intensive")]:
        ...
        print(
            f"  Type {task_type:+d} {label_text:18s}: "
            f"generated={len(generated):2d}, "
            f"assigned={len(assigned):2d}, "
            f"on-time={len(on_time):2d}, "
            f"late={len(late):2d}, "
...
def main(num_tasks=NUM_TASKS, num_uavs=NUM_UAVS):
    ...
    result = DPSOScheduler().schedule(uavs, tasks)

    print_summary(result, tasks, uavs, label="DPSO")
    print_routes(result)
